...
Java Data Obfuscation Through Masking (J-DOTM) is a utility to Obfuscate any kind of sensitive data. Data obfuscation is the process of replacing sensitive information with data that looks like real production data, making it useless to malicious actors. _Data masking_, _encryption_, and _tokenization_ are three common data obfuscation techniques.

This utility provides Data obfuscation using data masking (also called _data anonymization_) techniques (_replacing the real data with randomly generated fake data, the data values are changed, but the format remains the same_). There are following majors advantages with this technique: -

1. Masked data is usable in its obfuscated form
2. It also maintains the data integrity
3. Mask values cannot be recovered.

This utility can be configured for any language/multilingual characters and provides fine grain control over any character set and obfuscation rules can easily be modified/configured. You may create as many random combinations as you want. The default is set to 200. The random count combination allows the same character to yield a different random output as specified by the count itself (_without impacting the data integrity_), so it is impossible for the malicious actors to hack the system. If you make the random combination count really big, it will demand more memory footprints, without impacting performance.

You need Java 8 or above for this utility to work. Download and copy the utility class into your workspace (_if needed, rename the package name to your desired project/folder structure_). This utility consists of a single class file that needs no other dependencies and is self-sufficient. If your organization has any security restrictions, you need not worry about it as you will not be using any other third-party jars that may otherwise impose security alerts. This is indeed a very lightweight utility that should fulfill business needs. 

Any values you wish to configure will be passed to this utility as a constructor argument so you can maintain the configuration values the way you wish without any additional overhead. For data integrity, you must remember to use exactly the same version of this utility along with exactly the same parameters passed through its constructor. At any point in time, you take the revised version or modify its constructor arguments/parameters, you must ensure to regenerate the obfuscated values across the entire dataset. You must have to pass a long seed value through its constructor argument (_the seed value is the only mandatory value_) and the seed value must be kept hidden/secret by the admin for the production ready data as output of the generated dataset will be unique to this seed value.

This utility is highly performant, it is thread-safe and you can obfuscate (_any kind of data such as email, first/last name, phone number etc_) billions of records in just few minutes.
