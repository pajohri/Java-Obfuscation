...
You need Java 8 or above for this utility to work. Download and copy the utility class into your workspace. This utility consists of a single class file that needs no other dependencies and is self-sufficient. If your organization has any security restrictions, you need not worry about it as you will not be using any other third-party jars that may otherwise impose security alerts. This is indeed a very lightweight utility that should fulfill business needs. Any values you wish to configure will be passed to this utility as a constructor argument so you can maintain the configuration values the way you wish without any additional overhead.

You can configure the utility by passing the configuration argument through its constructor. For data integrity, you must remember to use exactly the same version of this utility along with exactly the same parameters passed through its constructor. At any point in time, you take the revised version or modify its constructor arguments/parameters, you must ensure to regenerate the obfuscated values across the entire dataset. You must have to pass a long seed value through its constructor argument (_the seed value is the only mandatory value_) and the seed value must be kept hidden/secret by the admin for the production ready data as output of the generated dataset will be unique to this seed value.
